...
/// or disable UI for features the host never wires (e.g. image pasting in
/// Ad-hoc Notes).
struct NoteWebViewCapabilities {
    var pasteImage: Bool
    var fold: Bool

    static let all = NoteWebViewCapabilities(pasteImage: true, fold: true)
    static let none = NoteWebViewCapabilities(pasteImage: false, fold: false)

    var json: String {
        "{\"pasteImage\":\(pasteImage),\"fold\":\(fold)}"
    }
}
...
protocol NoteWebViewHost: AnyObject {
    ...
    var getEditorContext: ((@escaping (Result<String, Error>) -> Void) -> Void)? { get set }
