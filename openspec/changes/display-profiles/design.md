...
既存の Registered Note では `NoteConfig` → `defaults` → ハードコードデフォルトの resolve チェーンで設定を解決している。Ad-hoc Note でも同様のパターンを、profile 経由で適用する。
...
- `defaults` を Registered Note / Ad-hoc Note 共通の基底設定として統一する
...
見た目設定（color, transparency, fontSize, position, autoHide, title）は共通。`defaults` が全 Note の基底設定となる。
...
### 3. 設定の resolve 順序: `defaults` を共通基底に

```
Registered Note: note inline → defaults → hardcoded
Ad-hoc Note:     profile     → defaults → hardcoded
```

`defaults` は全 Note 共通の基底設定。Registered Note と Ad-hoc Note が同じ fallback チェーンを共有することで、設定の一貫性を保つ。
...
notes:
  - path: ~/todo.md
    color: blue           # override defaults
    hotkey: cmd+shift+t

display:
  profiles:
    notify:
      color: pink         # override defaults
      auto_hide: true
      hotkey: cmd+shift+n
```

`DisplayProfile` の resolve メソッドは `NoteDefaults?` を引数に取る（`NoteConfig` と同じパターン）。
...
- **`defaults` スコープ拡張の影響** → 既存の `defaults` 変更が Ad-hoc Note にも波及するようになる。ただし Ad-hoc Note は profile で個別設定するのが基本なので、実影響は小さい
