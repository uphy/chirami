...
- Run on-device speech-to-text using **sherpa-onnx**. Default model `sherpa-onnx-sense-voice-zh-en-ja-ko-yue-2024-07-17`, auto-downloaded on first use to `~/.local/state/chirami/models/sherpa-onnx/`. Manual model paths also supported.
- Stream confirmed transcription chunks into the code block body in the format `[YYYY-MM-DD HH:MM:SS] You: text` / `[YYYY-MM-DD HH:MM:SS] Others: text`. Text remains plain Markdown — editable, greppable, readable in any editor.
...
### New Capabilities
- `transcript-block`: In-note speech-to-text capture. Covers the `transcript` code block, recording state machine, device selection fallback, sherpa-onnx integration, model management, and the plain-text output format.
...
- **New Swift code**: Audio capture stack (`AVAudioEngine` for mic, `CATapDescription` for system audio), `sherpa-onnx` integration, streaming transcription manager, device enumeration, permissions flow, per-block session coordinator.
...
- **New dependency**: sherpa-onnx native runtime. Adds native libraries and model download management; models are downloaded at runtime, not bundled.
...
- **State**: New `~/.local/state/chirami/models/sherpa-onnx/` directory for downloaded models. New optional entries in `state.yaml` for last-used mic / system-audio process.
