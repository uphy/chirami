  ...
  adhoc:
    profiles:
      agent:
        color_scheme: blue
  ```
- **THEN** agent profile の color_scheme は `blue`（profile で指定）、その他はハードコードデフォルト（transparency: 0.9, fontSize: 14, position: fixed）

#### Scenario: No display section

- **WHEN** config.yaml に `adhoc` セクションが存在しない
...
- **WHEN** `chirami display --profile nonexistent "Hello"` を実行し、config.yaml の `adhoc.profiles` に `nonexistent` が存在しない
...
#### Scenario: Multiple windows without saved state

- **WHEN** 同一 profile で保存済みの位置がない Ad-hoc Note を連続して表示する
- **THEN** 各ウィンドウは現在表示中の同一 profile ウィンドウと重ならないよう `(20px, 20px)` ずつオフセットされる

#### Scenario: Saved state overlaps with visible window

- **WHEN** `--id s1` が画面中央に保存済みで、同一 profile の `--id s2` が既に画面中央に表示中の状態で `s1` を表示する
- **THEN** `s1` の保存済み位置が `s2` と重なるため、`(20px, 20px)` オフセットされて配置される

#### Scenario: Saved state with no overlap

- **WHEN** `--id s1` が (100, 200) に保存済みで、同一 profile の他ウィンドウが (100, 200) に表示されていない
- **THEN** `s1` は保存済みの (100, 200) にそのまま表示される
...
`--id` 付きで表示された Ad-hoc Note の位置・サイズを state.yaml に保存する。キーは `adhoc:<id>` 形式。次回同じ id で表示する際に復元する。

#### Scenario: State saved on window move

- **WHEN** id `session1` の Ad-hoc Note をドラッグして位置を変更
- **THEN** state.yaml の `windows.adhoc:session1` に新しい位置が保存される

#### Scenario: State restored on re-display

- **WHEN** state.yaml に `adhoc:session1` の位置情報が保存されている状態で `chirami display --id session1 "content"` を実行
